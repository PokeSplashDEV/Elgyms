...
public class GetTeam {
	public LiteralCommandNode<ServerCommandSource> build() {
		return CommandManager.literal("getteam")
				.requires(ctx -> {
	...
	public int run(CommandContext<ServerCommandSource> context) {
		...
		Leader leader = gymConfig.getLeader(badges.getUuid());

		if (leader.getTeam() == null) {
			context.getSource().sendMessage(Text.literal("§c" + player + " has no team."));
			return 1;
		}
		...
		try {
			...
			PCStore pcStore = Cobblemon.INSTANCE.getStorage().getPC(context.getSource().getPlayer().getUuid());
			...
			for (JsonObject object : leader.getTeam()) {
				Pokemon pokemon = new Pokemon().loadFromJSON(object);
